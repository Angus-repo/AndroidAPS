...
import app.aaps.plugins.configuration.maintenance.storage.StorageType
...
class CloudDirectoryDialog @Inject constructor(
    ...
    fun showCloudDirectoryDialog(
        ...
        val isCloudSelected = currentType == StorageType.GOOGLE_DRIVE
        ...
        clearAction.setOnClickListener {
            ...
            val proceedClear: () -> Unit = {
                ...
                cloudStorageManager.setActiveStorageType(StorageType.LOCAL)
        ...
        val handleGoogleDriveClick = {
            ...
            dialog.dismiss()
            handleCloudSelection(activity, StorageType.GOOGLE_DRIVE, onCloudSelected, onStorageChanged)
        }
    ...
    private suspend fun startAuthFlow(
        ...
        try {
            ...
            if (authUrl == null) {
                ToastUtils.errorToast(activity, "Failed to start authorization")
                return
            }
    ...
    /** Show cloud folder selection */
    private fun showCloudFolderSelection(activity: DaggerAppCompatActivityWithResult, storageType: String) {
        activity.lifecycleScope.launch {
            try {
                val provider = cloudStorageManager.getProvider(storageType) ?: return@launch
                val folders = provider.listFolders()
                    ...
                    .setItems(folderNames.toTypedArray()) { _, which ->
                        when {
                            which == folderNames.size - 1 -> {
                                showCreateFolderDialog(activity, storageType)
                            }
                            else -> {
                                val selectedFolderId = folderIds[which]
                                provider.setSelectedFolderId(selectedFolderId)
                ...
                aapsLogger.error(LTag.CORE, "$LOG_PREFIX Error showing folder selection", e)
    ...
    private fun showCreateFolderDialog(activity: DaggerAppCompatActivityWithResult, storageType: String) {
        ...
        MaterialAlertDialogBuilder(activity, app.aaps.core.ui.R.style.DialogTheme)
            ...
            .setPositiveButton(rh.gs(R.string.create)) { _, _ ->
                ...
                if (folderName.isNotEmpty()) {
                    createFolder(activity, storageType, folderName)
                } else {
    ...
    private fun createFolder(activity: DaggerAppCompatActivityWithResult, storageType: String, folderPath: String) {
        activity.lifecycleScope.launch {
            try {
                val provider = cloudStorageManager.getProvider(storageType) ?: return@launch
                ...
                for (folderName in pathParts) {
                    val folderId = provider.createFolder(folderName, currentParentId)
                    if (folderId != null) {
                ...
                provider.setSelectedFolderId(currentParentId)
                ToastUtils.infoToast(activity, rh.gs(R.string.folder_created_successfully, folderPath))
            } catch (e: Exception) {
                aapsLogger.error(LTag.CORE, "$LOG_PREFIX Error creating folder", e)
