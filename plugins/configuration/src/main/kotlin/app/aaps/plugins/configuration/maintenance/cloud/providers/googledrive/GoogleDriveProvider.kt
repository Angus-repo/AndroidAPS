...
class GoogleDriveProvider @Inject constructor(
    ...
    override suspend fun listFolders(parentId: String): List<CloudFolder> {
        ...
        return googleDriveManager.listFolders(parentId).map { driveFolder ->
            CloudFolder(
                ...
                parentId = parentId.takeIf { it != "root" }
            )
        }
    }
    ...
    override suspend fun listFiles(
        ...
        gLog("listFiles: folderId=$folderId, pageSize=$pageSize")
        // Use the paged listing from GoogleDriveManager
    ...
    override fun showFolderSelectionDialog(
        ...
        // This can be implemented to show a custom folder picker dialog
        // For now, we'll leave the default implementation
        gLog("showFolderSelectionDialog - using default implementation")
    }
    ...
    /**
     * Get the current storage type configured in GoogleDriveManager.
     * Note: This is internal as storage type should be managed via CloudStorageManager.
     */
    internal fun getCurrentStorageType(): String {
        return googleDriveManager.getStorageType()
    }

    /**
     * Set the storage type in GoogleDriveManager.
     * Note: This is internal as storage type should be managed via CloudStorageManager.
     */
    internal fun setCurrentStorageType(type: String) {
        googleDriveManager.setStorageType(type)
    }

    /**
     * Get all settings files (non-paged) 
     */
    override suspend fun listAllSettingsFiles(): List<CloudFile> {
        gLog("listAllSettingsFiles")
        return googleDriveManager.listSettingsFiles().map { driveFile ->
            CloudFile(
                id = driveFile.id,
                name = driveFile.name,
                mimeType = guessMimeType(driveFile.name)
