...
class CloudStorageManager @Inject constructor(
    ...
    /**
     * Test connection to the active cloud provider.
     * @return true if connection is successful, false if failed or no cloud provider active
     */
    suspend fun testActiveConnection(): Boolean {
        val provider = getActiveProvider()
        if (provider == null) {
            aapsLogger.debug(LTag.CORE, "$LOG_PREFIX No active cloud provider to test")
            return false
        }
