...
class ExportOptionsDialog @Inject constructor(
    ...
    fun showExportOptionsDialog(
        ...
        allCloudSwitch.setOnCheckedChangeListener { _, isChecked ->
            ...
            if (isChecked && !cloudConfigured) {
                ToastUtils.warnToast(activity, rh.gs(R.string.google_drive_connection_failed))
     ...
     */
    fun isLogEmailEnabled(): Boolean {
        val value = sp.getBoolean(PREF_LOG_EMAIL_ENABLED, true)
        aapsLogger.info(LTag.CORE, "$LOG_PREFIX ExportDestination: isLogEmailEnabled=$value")
        return value
    }
    ...
    fun isCsvLocalEnabled(): Boolean {
        val value = sp.getBoolean(PREF_CSV_LOCAL_ENABLED, true)
        aapsLogger.info(LTag.CORE, "$LOG_PREFIX ExportDestination: isCsvLocalEnabled=$value")
        return value
    }
